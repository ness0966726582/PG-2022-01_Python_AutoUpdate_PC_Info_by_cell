...
def getUsing_TYPE_IP_MAC():
    ...
    time = datetime.now().strftime('%Y-%m-%d')          #時間 
    hostname = socket.gethostname().strip(" ")                   #除去空白字元 #主機名稱  
    
    IP = socket.gethostbyname(socket.gethostname()).strip(" ")                   #除去空白字元
    check_IP=IP.strip(" ")                   #除去空白字元#待檢查IP資訊
    ...
    for x in range (0, int(row_end-1)):          #從0開始 逐行進行split
        ...
        if str(list[2].strip(" "))==str(IP):      #除去空白字元比對IP
            ...
            adapter=list[0].strip(" ")                   #除去空白字元
            MAC=list[1].strip(" ")                   #除去空白字元

            update_list = [time, hostname, IP,MAC]
            check_time = update_list[0].strip(" ")                   #除去空白字元
            check_hostname = update_list[1].strip(" ")                   #除去空白字元
            check_IP = update_list[2].strip(" ")                   #除去空白字元
            check_MAC = update_list[3].strip(" ")                   #除去空白字元
            break
